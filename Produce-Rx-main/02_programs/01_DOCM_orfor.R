...
setwd("~/Desktop/HMS/Post-PCE/E2/Produce-Rx-main/")
...
source("02_Programs/3_sim_function_orfor.R")
...
if (length(args) == 0) {
  ...
  n.cycle <- 60 #Number of Cycle Length: How long does the model run (i.e., analytic time horizon)
...
# Orforglipron dosing and discontinuation
# Assume everyone starts on 36mg (highest efficacy dose from trials)
orforglipron_dose <- 36  # mg

# Discontinuation rate due to AEs (from trial data - you'll need to extract this)
# From NEJM trials: extract overall discontinuation rate due to AEs
discontinuation_rate_annual <- 0.15  # Example - adjust based on trial data

# Policy effect sizes from trials
# BMI effects (kg) - from obesity trial at 72 weeks, then plateau
policy_effect_BMI_year1_sim <- rnorm(n.sim, -8.2, 1.2)  # From 72-week trial
policy_effect_BMI_plateau_sim <- policy_effect_BMI_year1_sim  # Plateau at this level

# LDL effects (mg/dL) - extract from CV risk factor paper
policy_effect_LDL_sim <- rnorm(n.sim, -15.2, 3.1)  # Example values - adjust from paper

# HbA1c effects (%) - from diabetes trial, applies only to diabetic population
policy_effect_HbA1c_DM_sim <- rnorm(n.sim, -1.8, 0.2)  # From 40-week diabetes trial
policy_effect_HbA1c_NoDM_sim <- rnorm(n.sim, -0.2, 0.1)  # Minimal effect in non-diabetics

# Direct CVD risk reduction (beyond metabolic effects)
# This would be a relative risk reduction - you may need to estimate this
policy_effect_CVD_RR_sim <- calc_nsims_rbeta(n.sim, 0.85, 0.05)  # 15% RR reduction

# Drug costs (annual)
c_orforglipron = rnorm(n.sim, 5400, 500)  # Estimated annual cost - adjust as needed
...
NHANES<- fread("01_Inputs/nhanes_final_mice_treatments.csv", stringsAsFactors = TRUE, data.table = FALSE)
...
variables <- c("SEQN", "WTINT2YR", "WTMEC2YR", "WT_TOTAL", "SDMVPSU", "SDMVSTRA", 
               ...
               "Total_Chol","HDL", "hld", "HBP", "SBP", "DBP", "HPT_Txt", "Smoking",
               "DM_family", "BMI", "height", "weight", "Glucose", "Trig","Ins","Private","Medicare","Medicaid","Dual")

NHANES <- NHANES[variables]

# Define weight-related comorbidities
NHANES$hypertension <- ifelse(NHANES$HBP == 1 | NHANES$SBP >= 130 | NHANES$DBP >= 80, 1, 0)
NHANES$dyslipidemia <- ifelse(NHANES$hld == 1 | NHANES$Total_Chol >= 240 | NHANES$HDL < 40 | NHANES$Trig > 150, 1, 0)  # Adjust thresholds as needed
NHANES$cvd <- NHANES$CVD_history

# Create weight-related comorbidity indicator
NHANES$weight_related_comorbidity <- ifelse(
  NHANES$Diabetes == 1 | 
    NHANES$hypertension == 1 | 
    NHANES$dyslipidemia == 1 | 
    NHANES$cvd == 1, 1, 0)

# 1.1.2 Select the target starting population to model (Age 40-79) 
NHANES_age40_79 <- subset(NHANES, Age >= 40 & Age < 80 & Diabetes == 0)
#NHANES_age20_79 <- subset(NHANES, Age >= 20 & Age < 80 & Diabetes == 0)

# If age and BMI with CVD criteria (diabetes included)
NHANES_age20_79 <- subset(NHANES,
                          Age >= 20 & Age < 80 &
                            (BMI >= 30 | 
                               (BMI >= 27 & BMI < 30 & weight_related_comorbidity == 1)))
# If age and BMI (no diabetes)
#NHANES_age20_79 <- subset(NHANES, Age >= 20 & Age < 80 & DIABETES == 0 & BMI >= 30)

# Only selects for people included in physical exam/lab weighted cohort:
cat("Checking for missing survey design variables...\n")
missing_survey <- is.na(NHANES_age20_79$SDMVPSU) | 
  is.na(NHANES_age20_79$SDMVSTRA) | 
  is.na(NHANES_age20_79$WT_TOTAL) |
  NHANES_age20_79$WT_TOTAL <= 0

if(sum(missing_survey) > 0) {
  cat("Removing", sum(missing_survey), "observations with missing survey design variables\n")
  NHANES_age20_79 <- NHANES_age20_79[!missing_survey, ]
}

NHANES_age20_79$Subject_ID <- c(1:nrow(NHANES_age20_79))

# This weighted cohort is total population: 128.6M which is similar to that in Hwang paper (126M)

# 1.1.3 Define the analytic dataset to carry it forward

if (n.sample == "ALL"){
  data_for_analysis <- NHANES_age20_79
} else {
  random_sample <- sample(1:nrow(NHANES_age20_79), n.sample, replace=F)
  data_for_analysis <- NHANES_age20_79[random_sample,]
}
...
RR_diet_disease <- fread("01_Inputs/final_diet_RRs_MainAnalysis v11.csv", stringsAsFactors = TRUE, data.table = FALSE)
...
Non_CVD_DM_mortality <- fread("01_Inputs/Non_DM_IHD_Stroke_Cause_Mortality (Annual probability) (2001-2016 & 85+ corrected).csv", stringsAsFactors = TRUE, data.table = FALSE)
...
stroke_mortality <- fread("01_Inputs/Stroke_Cause_Mortality (Annual probability) (2001-2016 & 85+ adjusted).csv", stringsAsFactors = TRUE, data.table = FALSE)
...
CHD_mortality <- fread("01_Inputs/IHD_Cause_Mortality (Annual probability) (2001-2016 & 85+ adjusted).csv", stringsAsFactors = TRUE, data.table = FALSE)
...
DM_mortality <- fread("01_Inputs/DM_Cause_Mortality (Annual probability) (2001-2016 & 85+ adjusted).csv", stringsAsFactors = TRUE, data.table = FALSE)
...
risk_factor_trend <- fread("01_Inputs/Risk_Factors_Trends_Summary (Final).csv", stringsAsFactors = TRUE, data.table = FALSE)
...
Prop_CHD <- fread("01_Inputs/Prop_CHD.csv", stringsAsFactors = FALSE, data.table = FALSE)
...
# Fill the base parameters first
for (i in 1:nrow(HCE_parameters)) {
  HCE_parameter_sim[i,] <- as.matrix(rnorm(n.sim, HCE_parameters$Beta[i], HCE_parameters$SE[i]))
}

# Apply orforglipron HbA1c effect on healthcare costs (using diabetic effect as primary)
HCE_parameter_sim_diabe <- HCE_parameter_sim
HCE_parameter_sim_diabe[8,] <- HCE_parameter_sim[8,] * (1 + policy_effect_HbA1c_DM_sim * 0.13)
...
design_sample = subset(design_all, Age >= 20 & Age < 80)
...
# =======
# THE FOLLOWING WORKED PRIOR TO BMI CRITERIA:
# =======
# Replace the existing calls with:
byvars <- c("age_cat","Female","Race", "pir_cat","edu")

# Fixed function calls - remove the undefined diff_timehoriz_sub parameter
summary_by <- calc_summary_by(byvars, diff_timehoriz)

write.csv(summary_by[[1]], paste("03_Output/01_processed/summary_by_", seed, scenario, n.cycle,  "yrs_",  Sys.Date(),  ".csv", sep = ""))

write.csv(summary_by[[2]], paste("03_Output/01_processed/pop_summary_by_", seed, scenario, n.cycle, "yrs_",  Sys.Date(), ".csv", sep = ""))

varkeep <- c("Total_cost_health", "Total_cost_societ", "effect_disc")
allsimsby <- calc_allsim_by(byvars, varkeep, diff_timehoriz)  # Fixed function call
write.csv(allsimsby, paste("03_Output/01_processed/pop_summary_allsim_bysub_", seed, scenario, n.cycle, "yrs_",  Sys.Date(), ".csv", sep = ""))
