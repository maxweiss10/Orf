...
run_sim <- function(s, intervention) {
  ...
  if (intervention == "Policy") {
    orforglipron_active <- rep(1, nrow(sim_out_t))  # Everyone starts on drug
  } else {
    orforglipron_active <- rep(0, nrow(sim_out_t))  # No one gets drug
  }
  ...
  for (t in 1:n.cycle) {
    ...
    # Handle drug discontinuation
    if (intervention == "Policy" & t > 1) {
      discontinue_this_cycle <- rbinom(nrow(sim_out_t), 1, discontinuation_rate_annual)
      orforglipron_active <- orforglipron_active * (1 - discontinue_this_cycle)
    }
    
    # Apply orforglipron BMI effects
    if (intervention == "Policy") {
      if (t <= 2) {  # Effect builds up over first 2 years (72 weeks)
        bmi_reduction <- policy_effect_BMI_year1_sim[s] * (t/2) * orforglipron_active
      } else {  # Plateau after 2 years
        bmi_reduction <- policy_effect_BMI_year1_sim[s] * orforglipron_active
      }
      sim_out_t[,"BMI"] <- sim_out_t[,"BMI"] + bmi_reduction
      sim_out_t[,"BMI"] <- update_risk_factor(sim_out_t, "BMI", "BMI", s)
    } else {
      sim_out_t[,"BMI"] <- update_risk_factor(sim_out_t, "BMI", "BMI", s)
    }
    
    # Apply secular trends to all risk factors (this happens AFTER orforglipron effects)
    ...
    # Truncate BMI to the interval 12-70 kg/m2 (AFTER all BMI effects are applied)
   ...
   # Direct orforglipron CVD risk reduction (beyond BMI effects)
   if (intervention == "Policy") {
     orforglipron_cvd_rr <- ifelse(orforglipron_active == 1, policy_effect_CVD_RR_sim[s], 1.0)
   } else {
     orforglipron_cvd_rr <- 1.0
   }
   
   # Keep only BMI effects on CVD risk
   RR_diff_total_CHD <- RR_diff_bmi_ihd * orforglipron_cvd_rr
   RR_diff_total_Stroke <- RR_diff_bmi_tstk * orforglipron_cvd_rr
  ...
  p.CHD_recurrent <-  rep(as.numeric(Prop_CHD[2,data_for_analysis[,"DEMO"]]), n.loop)
   ...
   sim_out_t[,"Obesity"] <- ifelse(sim_out_t[,"BMI"] < 30, 0, 
    ...
    if (intervention == "Policy") {
      Drug_cost_t1 <- ifelse(sim_out_t[,"state"] != "Death" & orforglipron_active == 1, c_orforglipron[s]/n.cycle, 0)
    } else {
      Drug_cost_t1 <- 0
    }
    
    # Update the cost variables
    sim_out_t[,"Food_cost"] <- sim_out_t[,"Food_cost"] + Drug_cost_t1
    sim_out_t[,"Food_cost_disc"] <- sim_out_t[,"Food_cost_disc"] + Drug_cost_t1/((1+beta_cost)^(t-1))
    
    # Discount effects and costs
    #ppp: add adiministrative costs to the model
   
    # Drug costs for orforglipron (no special admin costs needed)
    if (intervention == "Policy") {
      Drug_cost_t1 <- ifelse(sim_out_t[,"state"] != "Death" & orforglipron_active == 1, 
                             c_orforglipron[s]/n.cycle, 0)
    } else {
      Drug_cost_t1 <- 0
    }
    
    # Update cost variables (keeping Food_cost variable name for compatibility)
    sim_out_t[,"Food_cost"] <- sim_out_t[,"Food_cost"] + Drug_cost_t1
    sim_out_t[,"Food_cost_disc"] <- sim_out_t[,"Food_cost_disc"] + Drug_cost_t1/((1+beta_cost)^(t-1))
    
    # Simplified total costs (no separate admin costs)
    sim_out_t[, "Total_cost_health"] <- sim_out_t[,"Food_cost_disc"] + sim_out_t[,"HCE_disc"]
    sim_out_t[, "Total_cost_societ"] <- sim_out_t[,"Food_cost_disc"] + sim_out_t[,"HCE_disc"] + sim_out_t[,"Prod_cost_disc"]
